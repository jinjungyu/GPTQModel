...
def benchmark_generation_speed(model, tokenizer, examples, generation_config, draft_model, compile):
    ...
    # import code; code.interact('generate_stats', local=dict(globals(), **locals()))
    # print(f"Output_Text : {tokenizer.decode(output_ids)}")
    print(f"generation speed: {total_tokens / total_seconds:.3f} tokens/s\n")
    # logger.info(
...
def main():
    ...
    parser.add_argument("--quantized_path_4bit", type=str)
    parser.add_argument("--quantized_path_2bit", type=str)
    parser.add_argument("--bit_config_json", type=str)
    ...
    parser.add_argument("--backend", default='BITBLAS', choices=['AUTO', 'TRITON', 'EXLLAMA', 'EXLLAMA_V2', 'MARLIN', 'BITBLAS'])
    ...
    # 4bit model load
    model_4bit = GPTQModel.from_quantized(
        args.quantized_path_4bit,
        quantize_config=quantize_config,
        use_safetensors=True,
        trust_remote_code=args.trust_remote_code,
        backend=get_backend(args.backend),
    )
    layers_4bit = model_4bit.model.model.layers
    
    # 2bit model load
    model_2bit = GPTQModel.from_quantized(
        args.quantized_path_2bit,
        quantize_config=quantize_config,
        use_safetensors=True,
        trust_remote_code=args.trust_remote_code,
        backend=get_backend(args.backend),
    )
    layers_2bit = model_2bit.model.model.layers
    
    del model_2bit, model_4bit
    ...
    #     args.max_new_tokens,
    # )
    # import code; code.interact(f'hf model', local=dict(globals(), **locals()))

    # added for torchao model
    if os.environ.get('FUSED'):
        pth_name = 'model_fused.pth'
    else:
        pth_name = 'model.pth'
    torchao.quantization.utils.recommended_inductor_config_setter()
    model = Transformer.from_name(args.model_name_or_path)
    model.load_state_dict(torch.load(Path(args.model_name_or_path) / pth_name, mmap=True, weights_only=True), assign=True)
    
    linear_name_map_dict = {
    ...
    def get_recursive_attr(root_attr, attr_name):
        attr_names = attr_name.split('.')
        new_root_attr = getattr(root_attr, attr_names[0])
        if len(attr_names) > 1:
            return get_recursive_attr(getattr(root_attr, attr_names[0]), '.'.join(attr_names[1:]))
        else:
            return new_root_attr
        
    def set_recursive_attr(root_attr, attr_name, newattr):
        attr_names = attr_name.split('.')
        if len(attr_names) > 1:
            set_recursive_attr(getattr(root_attr, attr_names[0]), '.'.join(attr_names[1:]), newattr)
        else:
            setattr(root_attr, attr_name, newattr)
        
    def calculate_average_bits(model, linear_cls):
        total_elems = 0
        total_bits = 0
        for module in model.modules():
            if isinstance(module, linear_cls):
                numel = module.infeatures * module.outfeatures
                total_elems += numel
                total_bits += numel * module.bits
        
        return total_bits / total_elems
        
    # read model config
    with open(args.bit_config_json, 'r') as f:
        config = json.load(f)
    
    # with open(args.bit_config_json.replace('.json', '_speed.json'), 'w') as f:
    
    # import code; code.interact(f'config = json.load(f)', local=dict(globals(), **locals()))
    if isinstance(config['archive'][0], Dict):
        config['archive'] = [config['archive']]
    
    for arch_datas in config['archive']:
        ppl_or_loss = arch_datas[1]
        if ppl_or_loss > 6.0:
            continue
        torch.compiler.reset()
        for linear_name, block_bits in arch_datas[0].items():
            for block_idx in range(len(block_bits)):
                block = model.layers[block_idx]
                bit = block_bits[block_idx]
                if bit == 2:
                    quant_linear = get_recursive_attr(layers_2bit[block_idx], linear_name)
                elif bit == 4:
                    quant_linear = get_recursive_attr(layers_4bit[block_idx], linear_name)
                else:
                    raise ValueError
                linear_name_ = linear_name_map_dict[linear_name]
                set_recursive_attr(block, linear_name_, quant_linear)
            
        print(f"Average Bits : {calculate_average_bits(model, quant_linear.__class__)}")
        ...
        if is_speculative:
            draft_model = _load_model(Path(args.draft_checkpoint_path) / 'model.pth', device, torch.half)
        else:
            draft_model = None
        ...
        # import code; code.interact('torch compile', local=dict(globals(), **locals()))
