...
def measure_time(model, input_args, num_iter, compile=False, warm_up=10):
    if compile:
...
def main(args):
    ...
    for name in name_map_dict:
        for bit in bit_candidate:
            ...
            times_dict['linear'][newname] = measure_time(linear, [input_x], args.num_iter, args.compile)
    ...
    times_dict['block'] = {}
    times_dict['edge'] = {}
    times_dict['block']['fp16'] = measure_time(block, input_args, args.num_iter, args.compile)
    bit_candidate = [2, 4]
    for lname1, lname2 in combination_names:
        for bit1, bit2 in [[2, 2], [2, 4], [4, 2], [4, 4]]:
            ...
            times_dict['block'][edge_name] = measure_time(block, input_args, args.num_iter, args.compile)
